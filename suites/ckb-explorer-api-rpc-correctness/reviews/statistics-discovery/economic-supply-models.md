...
| --- | --- | --- | --- | --- |
| `ECON-RPC-01` | 请求 `market_data` 首页并分别请求 `total_supply` 与 `circulating_supply` | 首页恰含 `total_supply` 与 `circulating_supply` 两个十进制字符串；三个 URL 分别返回各自缓存的有效供给快照，首页值与详情值允许因独立缓存的刷新时刻不同而存在差异，不要求跨 URL 完全一致 | 首页缺少供给字段、返回类型错误，或将正常缓存时间差误报为供给错误 | P0 |
