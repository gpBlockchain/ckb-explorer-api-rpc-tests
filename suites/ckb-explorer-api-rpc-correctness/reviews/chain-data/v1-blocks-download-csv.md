...
- 成功结果：CSV 表头和每行列数稳定，行集合等于过滤条件对应的同网络规范链区块集合并按高度降序排列；RPC 十六进制整数无损转换，奖励由 Shannon 精确换算为 CKB，矿工地址按对应网络编码，UTC 日期由毫秒时间戳确定转换。
...
| `BLOCKS-CSV-RPC-06` | 在公开主网和测试网分别不传过滤条件，并另选一个匹配超过 500 个区块的过滤范围 | 两种请求均只返回匹配集合中高度最新的 500 个区块并按高度降序排列 | 默认下载长期停留在创世区块附近，或大范围下载截取错误的一端导致新数据缺失 | P1 |
...
- 无；9 条用例均已确认，超过 500 个匹配区块时保留高度最新的 500 个并按高度降序排列。
