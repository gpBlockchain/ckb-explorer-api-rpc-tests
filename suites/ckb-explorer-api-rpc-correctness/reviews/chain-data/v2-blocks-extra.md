...
| `V2-NODE-VERSIONS-RPC-04` | Cellbase message 出现多位 major/patch、预发布或构建标识，例如 `10.12.13`、`0.103.10`、`0.103.0-rc1` | 待确认：版本键应保留完整语义版本，还是固定采用当前首个 `\d\.\d+\.\d` 匹配结果；一旦确定，所有同格式区块必须稳定归入同一键且不得产生静默截断碰撞 | 当前正则从多位 major 中间开始匹配、截断多位 patch/预发布标识，导致不同真实版本被合并 | P1 |
...
- `V2-NODE-VERSIONS-RPC-04`：`ckb_node_versions` 遇到多位 major/patch、预发布标识或其他合法节点版本格式时，是返回 Cellbase message 中的完整语义版本，还是保留源码当前 `\d\.\d+\.\d` 正则的截断或无法匹配行为。
- 请确认新增 `V2-NODE-VERSIONS-RPC-02` 至 `V2-NODE-VERSIONS-RPC-05` 及按当前源码明确最多 43 个 Epoch 窗口后的 `V2-NODE-VERSIONS-RPC-01` 可作为边界、结构排序、版本格式和 Epoch 滚动评审依据；本轮只补充测试点，不进入自动化门禁。
