...
| --- | --- | --- | --- | --- |
| `PENDING-RPC-01` | - [x] 在公开主网和测试网分别用同网络 CKB RPC `get_raw_tx_pool(true)` 包围一次列表观测，且目标交易在两次快照中保持为 pending 或 proposed | 每个 API `transaction_hash` 在稳定 RPC 交易池中恰好出现一次，API `transaction_fee` 等于对应 RPC `fee` 十六进制无损转换后的 Shannon 整数；主网、测试网分别给出结论 | 索引了错误网络、遗漏或重复待处理交易，或手续费进制和单位写错 | P0 |
| `PENDING-RPC-02` | - [x] 对 RPC 快照中保持稳定且被 API 返回的待处理交易读取列表项 | 列表项只包含 `transaction_hash`、`capacity_involved`、`transaction_fee`、`created_at`、`create_timestamp`；`create_timestamp` 等于 `created_at` 向下取整到毫秒的 Unix 时间，哈希和手续费与同一 RPC 交易一致 | 字段投影串行化漂移、秒和毫秒混淆，或把其他交易的属性拼到当前交易 | P0 |
...
| `PENDING-RPC-05` | - [x] 在稳定集合含不同创建时间的交易时分别请求 `sort=time`、`sort=time.asc` 和 `sort=time.desc` | `time` 与 `time.asc` 都按 API `created_at` 升序，`time.desc` 按其降序；相同时间以内部交易 ID 降序稳定打破并列，空值始终位于末尾 | 时间排序方向、默认方向或并列顺序漂移导致分页抖动 | P1 |
| `PENDING-RPC-06` | - [x] 在稳定集合含不同 RPC 手续费的交易时分别请求 `sort=fee`、`sort=fee.asc` 和 `sort=fee.desc` | `fee` 与 `fee.asc` 都按已由 RPC 核对的 `transaction_fee` 升序，`fee.desc` 按其降序；并列按内部交易 ID 降序，空值始终位于末尾 | 手续费排序使用错误单位或字段，或并列项跨页重复和遗漏 | P1 |
...
| `PENDING-RPC-08` | - [x] 请求 `sort=time.abcd`、未知排序字段及大小写不同的合法方向 | 非法或缺失方向按升序处理，未知字段回退到内部交易 ID；合法 `ASC`、`DESC` 大小写不敏感，任何值都不会成为自由 SQL 排序表达式 | 畸形排序造成服务错误、非确定顺序，或排序参数进入 SQL | P2 |
...
| `PENDING-RPC-12` | - [x] 同源 RPC 的 pending、proposed 均为空且 Explorer 缓存已刷新时调用列表和计数接口 | 列表返回 `data=[]`、`meta.total=0` 和请求采用的受限 `meta.page_size`，计数返回整数 `data=0` | 空池被当成错误、返回残留交易，或列表与计数的零值结构不一致 | P1 |
| `PENDING-RPC-13` | - [x] 任一网络的包围式 RPC 快照发生交易进入、离开或 pending/proposed 转换，或 RPC 方法和结果不可用 | 该网络本次依赖精确成员或计数的断言标记为事实基准不可用，不判为 API 数据错误；另一网络独立执行 | 交易池天然变化或 RPC 局部故障制造间歇性误报并连带污染另一网络 | P1 |
...
| `PENDING-RPC-20` | - [ ] 在稳定且至少足以产生两种不同结果的集合上，于同一 10 秒缓存新鲜期内交错请求不同 `page`、`page_size`、`sort` 和排序方向组合 | 每个响应的 `data` 顺序、切片及 `meta.page_size` 都只对应本次查询参数；重复同一 URL 可以命中其缓存，不同查询组合不得复用其他组合的响应体或元数据 | 公共缓存键忽略查询字符串，导致不同页码、页长或排序请求相互污染并返回错误交易集合 | P1 |
| `PENDING-RPC-21` | - [ ] 在交易池稳定时分别不带参数和携带合法、越界及畸形的 `page`、`page_size`、`sort` 参数请求计数接口 | 所有请求均返回 HTTP `200` 和相同的 JSON 整数 `data`，其值仍只等于数据库 pending 交易数；列表专用查询参数不参与过滤、分页或排序，也不触发参数错误 | 共用分页前置处理错误地改变计数、把 count 当作列表分页结果，或畸形列表参数导致计数接口异常 | P2 |
...
- 请确认新增 `PENDING-RPC-20` 可作为列表接口在缓存新鲜期内隔离不同分页和排序查询的评审依据；本轮只补充测试点，不进入自动化门禁。
- 请确认新增 `PENDING-RPC-21` 可作为 count 接口忽略列表专用分页和排序参数的评审依据；本轮只补充测试点，不进入自动化门禁。
